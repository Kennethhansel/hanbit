...
// ==========================================
// 1. LOGIKA PROSES DATA BRAND (CRUD + UPLOAD LOGO + HAPUS MASSAL)
// ==========================================
if (isset($_POST['tambah_brand'])) {
    ...
    if (!is_dir('../customer/images/logo/')) { mkdir('../customer/images/logo/', 0777, true); }
...
if (isset($_POST['edit_brand'])) {
    ...
    } else { $query = "UPDATE laptop_brands SET nama_brand = '$nama_brand' WHERE id_brand = $id_brand"; }
    mysqli_query($koneksi, $query);
    header("Location: master_laptop.php?status=brand_diperbarui");
    exit;
}

// ENGINE BARU: Eksekusi Hapus Massal Brand Terpilih via Checkbox
if (isset($_POST['eksekusi_hapus_brand_massal'])) {
    if (!empty($_POST['brands_id_hapus'])) {
        ...
        foreach ($_POST['brands_id_hapus'] as $id_brand_hapus) {
            ...
            // Validasi integritas relasi foreign key ke tabel series
        ...
        if ($gagal_hitung > 0) {
            header("Location: master_laptop.php?status=brand_gagal_relasi");
        } else {
            header("Location: master_laptop.php?status=brand_terhapus");
        }
        exit;
    }
}
...
// ==========================================
// 2. LOGIKA PROSES DATA SERIES (CRUD + HAPUS MASSAL)
// ==========================================
if (isset($_POST['tambah_series'])) { 
    ...
    if (!is_dir('../customer/images/series/')) { mkdir('../customer/images/series/', 0777, true); }
...
if (isset($_POST['edit_series'])) {
    ...
    } else { $query = "UPDATE laptop_series SET id_brand = $id_brand, nama_series = '$nama_series' WHERE id_series = $id_series"; }
    mysqli_query($koneksi, $query);
    header("Location: master_laptop.php?status=series_diperbarui");
    exit;
}

// ENGINE BARU: Eksekusi Hapus Massal Series Terpilih via Checkbox
...
$brands_array = [];
$result_brand_dropdown = mysqli_query($koneksi, "SELECT * FROM laptop_brands ORDER BY nama_brand ASC");
while($b_row = mysqli_fetch_assoc($result_brand_dropdown)) { $brands_array[] = $b_row; }
...
<head>
    ...
    <title>Hanbit Admin - Kelola Master Laptop</title>
        ...
        <div class="max-w-full mx-auto px-2 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 min-h-[56px]">
                <div>
                    ...
                    <p class="text-xs text-slate-400 font-medium">Atur registrasi nama merek brand beserta varian seri laptop konsumen Hanbit Labs.</p>
                </div>
                <div class="flex items-center gap-2.5 shrink-0">
                    ...
                    <button type="button" onclick="bukaModalBrand('tambah')" class="bg-black hover:bg-slate-900 text-white font-bold px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider transition shadow-sm flex items-center gap-1.5">
                        <i class="fas fa-plus text-[10px]"></i> Brand
                    </button>
                    <button type="button" onclick="bukaModalSeries('tambah')" class="bg-[#facc15] hover:bg-[#eab308] text-slate-955 font-black px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider transition shadow-sm flex items-center gap-1.5">
            ...
            <?php if (isset($_GET['status'])): ?>
                <div class="text-xs font-bold px-4 py-3 rounded-xl border bg-white shadow-sm">
                    <?php
                    if ($_GET['status'] == 'brand_sukses') echo "<span class='text-emerald-600'>✅ Brand baru & file logo berhasil ditambahkan!</span>";
                    elseif ($_GET['status'] == 'brand_diperbarui') echo "<span class='text-blue-600'>✅ Nama & logo brand berhasil diperbarui!</span>";
                    elseif ($_GET['status'] == 'brand_terhapus') echo "<span class='text-rose-600'>🗑️ Data brand terpilih berhasil dibersihkan dari database.</span>";
                    elseif ($_GET['status'] == 'brand_gagal_relasi') echo "<span class='text-red-600'>❌ Gagal menghapus beberapa brand! Masih ada tipe series laptop aktif yang terikat dengan merek tersebut.</span>";
                    elseif ($_GET['status'] == 'series_sukses') echo "<span class='text-emerald-600'>✅ Tipe series baru & file foto berhasil disimpan!</span>";
                    elseif ($_GET['status'] == 'series_diperbarui') echo "<span class='text-blue-600'>✅ File foto & konfigurasi seri laptop berhasil diperbarui!</span>";
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                ...
                <div class="xl:col-span-4 bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <form id="form_brand_massal" action="master_laptop.php" method="POST" class="overflow-x-auto">
                            ...
                            <thead>
                                <tr class="bg-slate-100/60 text-slate-400 font-bold uppercase border-b border-gray-100 select-none">
                                    <th class="p-4 w-12 text-center">
                                        <input type="checkbox" id="master_check_brand" onclick="toggleSemuaBrand(this)" class="w-4 h-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer">
                                    </th>
                                    <th class="p-4 w-16 text-center">Logo</th>
                                    <th class="p-4">Nama Brand</th>
                                    <th class="p-4 text-center w-14">Edit</th>
                                </tr>
                            </thead>
                                ...
                                <?php while($row_b = mysqli_fetch_assoc($result_brand)): ?>
                                    <tr class="hover:bg-slate-50/60 transition">
                                        <td class="p-4 text-center">
                                            <input type="checkbox" name="brands_id_hapus[]" value="<?= $row_b['id_brand']; ?>" onchange="hitungBrandTerpilih()" class="check_brand_child w-4 h-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer">
                                        </td>
                                        <td class="p-3 text-center">
                                            <?php $src_logo = (strpos($row_b['logo'], 'images/') !== false) ? '../customer/' . $row_b['logo'] : '../customer/images/logo/' . $row_b['logo']; ?>
                                            <img src="<?= $src_logo; ?>?v=<?= time(); ?>" class="w-8 h-8 object-contain rounded border bg-white mx-auto shadow-xs" onerror="this.src='../customer/images/logo/default.png'">
                                        </td>
                                        <td class="p-3 font-bold text-slate-900 uppercase tracking-wide text-[11px]"><?= htmlspecialchars($row_b['nama_brand']); ?></td>
                                        <td class="p-3 text-center">
                        ...
                        <div id="modal_hapus_brand_massal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
                            <div class="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-gray-100 space-y-4 text-center">
                                ...
                                <div class="space-y-1">
                                    <h3 class="text-sm font-black uppercase text-slate-900 tracking-wide">Hapus Merek Terpilih</h3>
                                    <p class="text-xs font-semibold text-slate-400 leading-relaxed">Apakah Anda yakin menghapus permanen (<span id="text_brand_total" class="text-red-500 font-bold">0</span> merek) dari database? Brand yang terikat dengan tipe seri aktif otomatis akan dilewati.</p>
                                </div>
                                <div class="pt-2 flex gap-2">
                                    <button type="button" onclick="tutupModalHapusBrand()" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-2.5 rounded-xl transition uppercase tracking-wider text-[10px]">Batal</button>
                                    <button type="submit" name="eksekusi_hapus_brand_massal" class="flex-1 bg-red-500 hover:bg-red-600 text-white font-black py-2.5 rounded-xl transition uppercase tracking-wider text-[10px] shadow-sm flex items-center justify-center">Ya, Hapus</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                <div class="xl:col-span-8 bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
                    <div class="p-4 border-b border-gray-100 bg-slate-50/50 space-y-3">
                        <div class="flex justify-between items-center">
                            <h3 class="text-xs font-extrabold uppercase text-slate-900 tracking-wider">Daftar Seri & Tipe Varian Laptop</h3>
                        ...
                        <div class="flex flex-wrap gap-1.5 pt-2 border-t border-gray-100 select-none">
                            <button type="button" id="btn_filter_semua" onclick="filterSariLaptop('semua', this)" class="btn-tab-brand bg-slate-900 text-white font-bold text-[10px] uppercase tracking-wide px-3 py-2 rounded-xl transition shadow-sm">
                                Semua Merek
                            </button>
                            <?php foreach ($brands_array as $brnd): ?>
                                <button type="button" onclick="filterSariLaptop('<?= $brnd['id_brand']; ?>', this)" class="btn-tab-brand bg-white hover:bg-slate-50 border border-gray-200 text-slate-600 font-bold text-[10px] uppercase tracking-wide px-3 py-2 rounded-xl transition">
                                    <?= htmlspecialchars($brnd['nama_brand']); ?>
                                </button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <form id="form_series_massal" action="master_laptop.php" method="POST" class="overflow-x-auto">
                        <table class="w-full text-left border-collapse text-xs">
                            <thead>
                                <tr class="bg-slate-100/60 text-slate-400 font-bold uppercase border-b border-gray-100 select-none">
                                    <th class="p-4 w-12 text-center">
                                        <input type="checkbox" id="master_check_series" onclick="toggleSemuaSeries(this)" class="w-4 h-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer">
                                    </th>
                                    <th class="p-4 w-20 text-center">Foto</th>
                                    <th class="p-4 w-32">Brand Merek</th>
                                    <th class="p-4">Nama Series Laptop</th>
                                    <th class="p-4 w-14 text-center">Edit</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 font-medium text-slate-700">
                                <?php if(mysqli_num_rows($result_series) == 0): ?>
                                    <tr><td colspan="5" class="p-8 text-center text-slate-400 font-bold italic">Belum ada seri laptop didaftarkan.</td></tr>
                                <?php else: ?>
                                    <?php while($row_s = mysqli_fetch_assoc($result_series)): ?>
                                        <tr data-brand-id="<?= $row_s['id_brand']; ?>" class="row-series-item hover:bg-slate-50/60 transition">
                                            <td class="p-4 text-center">
                                                <input type="checkbox" name="series_id_hapus[]" value="<?= $row_s['id_series']; ?>" onchange="hitungSeriesTerpilih()" class="check_series_child w-4 h-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer">
                                            </td>
                                            <td class="p-3 text-center">
                                                <?php $src_foto = (strpos($row_s['foto'], 'images/') !== false) ? '../customer/' . $row_s['foto'] : '../customer/images/series/' . $row_s['foto']; ?>
                                                <img src="<?= $src_foto; ?>?v=<?= time(); ?>" class="w-14 h-10 object-cover rounded border bg-white mx-auto shadow-xs" onerror="this.src='../customer/images/series/default.png'">
                                            </td>
                                            <td class="p-3">
                                                <span class="bg-slate-100 text-slate-800 text-[9px] font-black uppercase px-2.5 py-1 rounded-md border border-slate-200/50 tracking-wide">
                                                    <?= htmlspecialchars($row_s['nama_brand']); ?>
                                                </span>
                                            </td>
                                            <td class="p-3 font-bold text-slate-900 uppercase italic tracking-wide text-[11px]">
                                                <?= htmlspecialchars($row_s['nama_series']); ?>
                                            </td>
                                            <td class="p-3 text-center">
                                                <button type="button" onclick="bukaModalSeries('edit', '<?= $row_s['id_series']; ?>', '<?= htmlspecialchars($row_s['nama_series'], ENT_QUOTES); ?>', '<?= $row_s['id_brand']; ?>')" class="text-blue-500 hover:text-blue-700 text-sm p-1 transition"><i class="fas fa-edit"></i></button>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                <?php endif; ?>
                        ...
                        <div id="modal_hapus_series_massal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
                            <div class="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-gray-100 space-y-4 text-center">
                                <div class="w-12 h-12 bg-red-50 text-red-500 rounded-full flex items-center justify-center text-xl mx-auto border border-red-100">
                                    <i class="fas fa-exclamation-triangle"></i>
                                </div>
                                <div class="space-y-1">
                                    <h3 class="text-sm font-black uppercase text-slate-900 tracking-wide">Hapus Varian Tipe Terpilih</h3>
                                    <p class="text-xs font-semibold text-slate-400 leading-relaxed">Apakah Anda yakin menghapus permanen (<span id="text_series_total" class="text-red-500 font-bold">0</span> tipe) varian laptop yang dicentang?</p>
                                </div>
                                <div class="pt-2 flex gap-2">
                                    <button type="button" onclick="tutupModalHapusSeries()" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-2.5 rounded-xl transition uppercase tracking-wider text-[10px]">Batal</button>
                                    <button type="submit" name="eksekusi_hapus_series_massal" class="flex-1 bg-red-500 hover:bg-red-600 text-white font-black py-2.5 rounded-xl transition uppercase tracking-wider text-[10px] shadow-sm flex items-center justify-center">Ya, Hapus</button>
                                </div>
                            </div>
                        </div>
                    </form>
        ...
        <div class="bg-white rounded-[1.5rem] max-w-sm w-full p-6 shadow-2xl border border-gray-100 space-y-4">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs font-semibold">
                ...
                <div class="flex justify-end gap-2 pt-3 border-t">
                    <button type="button" onclick="tutupModalBrand()" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold px-4 py-2 rounded-xl transition uppercase tracking-wider text-[10px]">Batal</button>
                    <button type="submit" name="tambah_brand" id="btn_submit_brand" class="bg-black hover:bg-slate-900 text-white font-bold px-4 py-2 rounded-xl transition uppercase tracking-wider text-[10px]">Simpan Brand</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal_series" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
        <div class="bg-white rounded-[1.5rem] max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs font-semibold">
                ...
                <div class="flex justify-end gap-2 pt-3 border-t">
                    <button type="button" onclick="tutupModalSeries()" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold px-4 py-2 rounded-xl transition uppercase tracking-wider text-[10px]">Batal</button>
                    <button type="submit" name="tambah_series" id="btn_submit_series" class="bg-[#facc15] hover:bg-[#eab308] text-slate-950 font-black px-4 py-2 rounded-xl transition uppercase tracking-wider text-[10px]">Simpan Series</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function filterSariLaptop(brandId, btnTarget) {
            ...
            semuaTab.forEach(tab => {
                tab.className = "btn-tab-brand bg-white hover:bg-slate-50 border border-gray-200 text-slate-600 font-bold text-[10px] uppercase tracking-wide px-3 py-2 rounded-xl transition";
            });

            btnTarget.className = "btn-tab-brand bg-slate-900 text-white font-bold text-[10px] uppercase tracking-wide px-3 py-2 rounded-xl transition shadow-sm";
            ...
            semuaRow.forEach(row => {
                ...
                if (brandId === 'semua' || targetId === brandId) {
                    ...
                    // Reset centangan pada baris yang tersembunyi agar tidak ikut terhapus salah sasaran
        ...
        // ==========================================
        // JS ENGINE BARU: LOGIKA CHECKBOX BRAND (LEFT)
        // ==========================================
        ...
        function bukaModalHapusBrand() {
            ...
            const m = document.getElementById('modal_hapus_brand_massal');
        ...
        // ==========================================
        // JS ENGINE BARU: LOGIKA CHECKBOX SERIES (RIGHT)
        // ==========================================
        ...
        function hitungSeriesTerpilih() {
            ...
            if(totalTerpilih > 0) {
                ...
                btnHapus.classList.remove('scale-100', 'opacity-100'); btnHapus.classList.add('scale-95', 'opacity-0');
                setTimeout(() => { btnHapus.className = btnHapus.className.replace('flex', 'hidden'); btnHapus.classList.add('hidden'); }, 300);
        ...
        function bukaModalHapusSeries() {
            ...
            const m = document.getElementById('modal_hapus_series_massal');
            m.classList.remove('hidden'); m.classList.add('flex');
        }
        ...
        // ==========================================
        // SCRIPT MODAL FORM MANAGEMENT (BRAND & TIPE)
        // ==========================================
